...
class BERT4RecDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            ...
            input_ids = items[:-2] + [self.args.mask_id]
            target_pos = items[1:-1] + [self.args.mask_id]
            answer = [items[-2]]

        elif self.data_type == "test":
            input_ids = items[:-1] + [self.args.mask_id]
            target_pos = items[1:] + [self.args.mask_id]
            answer = [items[-1]]
        else:
            input_ids = items[:] + [self.args.mask_id]
            target_pos = items[:] + [self.args.mask_id] # will not be used
            answer = []
